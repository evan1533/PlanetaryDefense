...
{
	public TankFactory(int px, int py, int pi, int pj, boolean friendly,int[][] nodeGraf, int myBase, int enemBase)
	{							//Prod rate								  res cost
		super(px,py,pi,pj,friendly, 500,  'T', nodeGraf, myBase, enemBase, 25, 50);
	}
	...
	public void developUnit()
	{
		...
		units.add(new Tank(1,x,y,nodeGraph,enemBase,friendly));
	}
	...
	public void developUnit(int num)
	{
		for(int i = 0;i<num;i++)
		{
			units.add(new Tank(1,0,0,nodeGraph,end,friendly));
		}
	}
